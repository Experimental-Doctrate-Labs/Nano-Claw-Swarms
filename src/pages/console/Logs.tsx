...
interface LogEntry { id: string; level: string; message: string; agent_name: string | null; workflow_name: string | null; run_id: string | null; created_at: string; }
const levelColors: Record<string, string> = { info: "text-blue-400", warn: "text-claw-gold", error: "text-destructive", debug: "text-muted-foreground" };
...
const Logs = () => {
  ...
  const fetchLogs = async () => {
    ...
    let query = supabase.from("logs").select("*").eq("org_id", currentOrg.id).order("created_at", { ascending: false }).limit(100);
  ...
  useEffect(() => { setLoading(true); fetchLogs(); }, [search, currentOrg?.id]);

  if (!currentOrg) {
    return <div className="glass-panel p-12 text-center"><FileText className="w-12 h-12 text-muted-foreground mx-auto mb-4 opacity-40" /><p className="text-muted-foreground">Create an organization first.</p></div>;
  }

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-foreground mb-1">Logs</h1><p className="text-sm text-muted-foreground">System and run logs</p></div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 text-center"><FileText className="w-12 h-12 text-muted-foreground mx-auto mb-4 opacity-40" /><p className="text-muted-foreground">No logs yet.</p></div>
      ) : (
