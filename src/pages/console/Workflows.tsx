...
interface WorkflowStep { agent_id: string; order: number; }
interface Workflow { id: string; name: string; description: string | null; workflow_type: string; steps: any; created_at: string; }
...
const Workflows = () => {
  ...
  const fetchData = async () => {
    ...
    const [wRes, aRes] = await Promise.all([
      supabase.from("workflows").select("*").eq("org_id", currentOrg.id).order("created_at", { ascending: false }),
      supabase.from("agents").select("id, name").eq("org_id", currentOrg.id),
    ]);
  ...
  const typeLabels: Record<string, string> = { sequential: "Sequential", concurrent: "Concurrent", hierarchical: "Hierarchical" };
    ...
    <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={addStep} disabled={agents.length === 0} className="text-xs text-claw-ember hover:underline disabled:opacity-50">+ Add Step</button>
                  </div>
                  {agents.length === 0 && <p className="text-xs text-muted-foreground">Create agents first to add steps.</p>}
                  <div className="space-y-2">
                    {form.steps.map((step, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        <span className="text-xs text-muted-foreground w-6">{idx + 1}.</span>
                        <select value={step.agent_id} onChange={(e) => {
                          const newSteps = [...form.steps]; newSteps[idx] = { ...step, agent_id: e.target.value }; setForm({ ...form, steps: newSteps });
                        ...
                        <button onClick={() => removeStep(idx)} className="text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {workflows.map((wf) => (
            <motion.div key={wf.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-5 group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center claw-border"><GitBranch className="w-4 h-4 text-claw-ember" /></div>
                  <div>
                ...
                {canEdit && (
                  <button onClick={() => handleDelete(wf.id)} className="p-1.5 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-3.5 h-3.5" /></button>
                )}
              </div>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {(wf.steps as any[])?.map((step: any, idx: number) => (
                  <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-muted text-xs text-muted-foreground">
                    <Bot className="w-3 h-3" />{getAgentName(step.agent_id)}
                  </span>
                ))}
              </div>
            </motion.div>
